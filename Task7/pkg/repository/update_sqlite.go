...
func (r *UpdateSQLite) IsTableHasAnyRows(table_name string) bool {
	...
	stmt, err1 := r.db.Prepare("SELECT name FROM sqlite_master WHERE type='table' AND name=?")
	if err1 != nil {
...
func (r *UpdateSQLite) WriteResultInKeywordTable(resultComicsSlice []Comic) error {
	...
	stmt, err := r.db.Prepare("INSERT OR REPLACE INTO keyword (number, url, keywords) VALUES (?, ?, ?)")
	if err != nil {
		return err
	}
	defer stmt.Close()
