...
func MakeJSONWithComicsData() (int, int) {
	...
	go signalHandler(c, resultMap, &written) // ловит и обрабатывает сигнал

	// Заполняем канал numbers номерами комиксов, которые нужно обработать
	countOfNewComics := findNumbersToGet(numbers, max_num)
	...
	writeResultInJSON(resultMap)    // Записываем данные в JSON
...
func signalHandler(c chan os.Signal, resultMap map[int]OutputData, written *[]int) {
	<-c // Ожидаем сигнал SIGINT
	// если прервана:
	fmt.Println("\nПрограмма прервана.")
	writeResultInJSON(resultMap) //записываем в json то, что успели обработать
...
func findNumbersToGet(numbers chan int, max_num int) int {
...
func IsDataBaseExist() bool {
	...
	db_file := config.ReadConfig().DB_file
...
func getDataOfOneComic(num int) InputData {
...
func writeResultInJSON(resultMap map[int]OutputData) {
